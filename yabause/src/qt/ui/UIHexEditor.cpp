...
UIHexEditor::UIHexEditor(QWidget *p) {
  ...
  QList<u32> startList;
  QList<u32> endList;
  tabList << "All" << "BIOS" << "LWRAM" << "HWRAM" << "CS0" << "CS1" << "CS2"
          << "68K RAM" << "VDP1 RAM" << "VDP1 FB" << "VDP2 RAM" << "VDP2 CRAM";
  ...
  endList << 0x07FFFFFF << 0x0017FFFF << 0x002FFFFF << 0x060FFFFF << 0x03FFFFFF
          << 0x04FFFFFF << 0x058FFFFF << 0x05AFFFFF << 0x05C7FFFF << 0x05CFFFFF
          << 0x05EFFFFF << 0x05F7FFFF;
...
QString UIHexEditor::getMemoryAsText() {
  UIHexEditorWnd *hexEditorWnd = (UIHexEditorWnd *)currentWidget();
  return hexEditorWnd->getMemoryAsText();
}

UIHexEditorWnd::UIHexEditorWnd(QWidget *p) : QAbstractScrollArea(p) {
  gapSizeAddrHex = 10;
  gapSizeHexText = 16;
  bytesPerLine = 16;
  bytesPerGroup = 2;
...
void UIHexEditorWnd::sliderUpdate(int value) { setCursorPos(cursorAddr); }
...
u32 UIHexEditorWnd::getStartAddress() { return startAddress; }
...
u32 UIHexEditorWnd::getEndAddress() { return endAddress; }

u32 UIHexEditorWnd::getAddress() { return cursorAddr; }
...
u8 UIHexEditorWnd::readByte(u32 addr) {
  if ((addr >= 0x05D00000 && addr < 0x05D80000) ||
    ...
    return MappedMemoryReadWordNocache(addr & (~0x1), NULL) >>
           ((1 - (addr & 0x1)) << 3, NULL);
  else
...
void UIHexEditorWnd::writeByte(u32 addr, u8 val) {
  if ((addr >= 0x05D00000 && addr < 0x05D80000) ||
      (addr >= 0x05F80000 && addr < 0x05FC0000)) {
    u16 word = MappedMemoryReadWordNocache(addr & (~0x1), NULL) &
               (0xFF << ((addr & 0x1) << 3));
...
void UIHexEditorWnd::autoScroll() {
  ...
  if (actPos < 0) {
    ...
    actPos = (s64)(((autoScrollDragPos.y() - 3) / fontHeight) +
                   (s64)verticalScrollBar()->value()) *
             2 * (s64)bytesPerLine;
  }
...
void UIHexEditorWnd::adjustSettings() {
  ...
  verticalScrollBar()->setRange(startAddress / bytesPerLine,
                                (endAddress / bytesPerLine) -
                                    (areaSize.height() / fontHeight) + 1);
...
void UIHexEditorWnd::keyPressCursor(QKeyEvent *event) {
  ...
  if (event->matches(QKeySequence::MoveToNextPage)) {
    ...
    setCursorPos((cursorAddr & ~1) +
                 (verticalScrollBar()->pageStep() * 2 * bytesPerLine));
    resetSelection(cursorAddr);
  }
  ...
  if (event->matches(QKeySequence::Copy)) {
    ...
    if (!textEdit) {
      for (u64 idx = getSelectionStart(); idx < getSelectionEnd(); idx++)
        result.append(
            QString("%1").arg(readByte(idx), 2, 16, QChar('0')).toUpper());
    } else {
      for (u64 idx = getSelectionStart(); idx < getSelectionEnd(); idx++)
...
void UIHexEditorWnd::keyPressEvent(QKeyEvent *event) {
  ...
  u64 posAddr = verticalScrollBar()->value() * bytesPerLine +
                (cursorY / fontHeight) * bytesPerLine + posX / 2;
...
void UIHexEditorWnd::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (event->buttons() & Qt::LeftButton) {
    ...
    QPoint pos = event->pos();
    QRect hexRect = QRect(posHex, yPosEdit, hexCharsInLine * fontWidth,
                          viewport()->rect().height());
    QRect textRect = QRect(posText, yPosEdit, viewport()->rect().width(),
                           viewport()->rect().height());
...
void UIHexEditorWnd::drawHeaderArea(QPainter *painter, int left, int top,
                                    int right) {
  ...
  for (int i = 0; i < (bytesPerLine / bytesPerGroup); i++) {
    ...
    if (i == 0)
      linePos = posHex - (gapSizeAddrHex / 2) +
                (i * ((bytesPerGroup * 2) + 1) * fontWidth);
    else
      linePos = posHex - (fontWidth / 2) +
                (i * ((bytesPerGroup * 2) + 1) * fontWidth) - 1;
    ...
    for (int j = 0; j < bytesPerGroup; j++) {
      ...
      if (offset == posX) {
        ...
        painter->fillRect(QRect(hexOffset, top, fontWidth * 2, fontHeight),
                          highLighted);
      } else
  ...
  for (int i = 0; i < bytesPerLine; i++) {
    if (i == posX) {
      painter->setPen(colHighlighted);
      painter->fillRect(
          QRect(xPosAscii, top - fontAscent, fontWidth, fontHeight),
          highLighted);
    } else
      painter->setPen(textColor);
    if (i == hoverX)
...
void UIHexEditorWnd::drawAddressArea(QPainter *painter, int firstLineIdx,
                                     u32 lastLineIdx, int yPosStart, u32 addr) {
  u32 posBa = ((cursorY - yPosEdit) / fontHeight) * bytesPerLine;

  painter->fillRect(QRect(posAddr, yPosStart - fontHeight,
                          posHex - (gapSizeAddrHex / 2), height()),
                    addrAreaColor);

  // Paint address area
  for (u32 lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
       lineIdx += bytesPerLine, yPos += fontHeight) {
    if (posBa == lineIdx) {
      painter->setPen(colHighlighted);
      painter->fillRect(QRect(posAddr, yPos - fontAscent,
                              posHex - (gapSizeAddrHex / 2), fontHeight),
                        highLighted);
    } else
      painter->setPen(textColor);
    ...
    QString address =
        QString("%1").arg(lineIdx + addr, 8, 16, QChar('0')).toUpper();
...
void UIHexEditorWnd::drawHexArea(QPainter *painter, int firstLineIdx,
                                 u32 lastLineIdx, int yPosStart, u32 addr) {
  ...
  if (this->isEnabled()) {
    for (u32 lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
      ...
      int xPos = posHex;
      for (int colIdx = 0;
           ((lineIdx + colIdx) < endAddress && (colIdx < bytesPerLine));
           colIdx++) {
        ...
        // Paint hex value
        text = QString("%1")
                   .arg(readByte(addr + lineIdx + colIdx - firstLineIdx), 2, 16,
                        QChar('0'))
                   .toUpper();
...
void UIHexEditorWnd::drawTextArea(QPainter *painter, int firstLineIdx,
                                  u32 lastLineIdx, int yPosStart, u32 addr) {
  if (isEnabled()) {
    // Paint ascii area
    for (u32 lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
...
void UIHexEditorWnd::paintEvent(QPaintEvent *event) {
  ...
  // Draw header
  drawHeaderArea(&painter, event->rect().left(), event->rect().top(),
                 event->rect().right());
  ...
  if (isEnabled()) {
    lastLineIdx =
        ((event->rect().bottom() / fontHeight) + fontHeight) * bytesPerLine;
    if (lastLineIdx > endAddress)
      lastLineIdx = endAddress;
  } else
    lastLineIdx = 1;
  int yPosStart = ((firstLineIdx) / bytesPerLine) * fontHeight +
  ...
  if (!hasFocus() || textEdit)
    painter.fillRect(cursorX, cursorY + fontHeight - 2, fontWidth * 2, 2,
                     this->palette().color(QPalette::WindowText));
  else if (blinkCursor && selEnd - selStart == 0)
    painter.fillRect(cursorX, cursorY, 2, fontAscent,
                     this->palette().color(QPalette::WindowText));

  if (!hasFocus() || !textEdit)
    painter.fillRect(cursorTextX, cursorTextY + fontHeight - 2, fontWidth, 2,
                     this->palette().color(QPalette::WindowText));
  else if (blinkCursor && selEnd - selStart == 0)
    painter.fillRect(cursorTextX, cursorTextY, 2, fontAscent,
                     this->palette().color(QPalette::WindowText));
}

void UIHexEditorWnd::resizeEvent(QResizeEvent *event) { adjustSettings(); }

bool UIHexEditorWnd::focusNextPrevChild(bool next) { return false; }
...
void UIHexEditorWnd::setCursorPos(s64 position) {
  ...
  cursorX = posHex + ((((x / nibblesPerGroup) * (nibblesPerGroup + 1)) +
                       (x % nibblesPerGroup)) *
                      fontWidth);
...
s64 UIHexEditorWnd::cursorPos(QPoint pos, bool toggleTextEdit) {
  ...
  if ((pos.x() >= posHex) &&
      (pos.x() < (posHex + hexCharsInLine * fontWidth)) &&
      pos.y() >= yPosEdit) {
    ...
    if ((x % (nibblesPerGroup + 1)) == 0)
      ...
      x = ((x / (nibblesPerGroup + 1)) * nibblesPerGroup) +
          (x % (nibblesPerGroup + 1));
    s64 y = (s64)(((pos.y() - yPosEdit - 3) / fontHeight) +
                  (s64)verticalScrollBar()->value()) *
            2 * (s64)bytesPerLine;
    result = x + y;
    if (toggleTextEdit)
      textEdit = false;
  } else if ((pos.x() >= posText) &&
             (pos.x() < (posText + (bytesPerLine + 1) * fontWidth)) &&
             pos.y() >= yPosEdit) {
...
s64 UIHexEditorWnd::cursorPos() { return cursorAddr; }
...
u64 UIHexEditorWnd::getSelectionStart() { return selStart; }

u64 UIHexEditorWnd::getSelectionEnd() { return selEnd; }
...
bool UIHexEditorWnd::saveMemory(QString filename, u32 startAddress,
                                u32 endAddress) {
...
QString UIHexEditorWnd::getMemoryAsText() {
  QString result;
  int height = viewport()->height();
  int linesPerPage = height / fontHeight;
  u32 pos = verticalScrollBar()->value();
  u32 startAddr = pos * bytesPerLine;
  u32 endAddr = startAddr + (linesPerPage * bytesPerLine);

  // Limit to valid address range
  if (startAddr < startAddress)
    startAddr = startAddress;
  if (endAddr > endAddress)
    endAddr = endAddress;

  // Generate text output
  for (u32 addr = startAddr; addr < endAddr; addr += bytesPerLine) {
    // Address column
    result += QString("%1: ").arg(addr, 8, 16, QChar('0')).toUpper();

    // Hex bytes column
    for (int i = 0; i < bytesPerLine; i++) {
      if (addr + i <= endAddr) {
        u8 byte = readByte(addr + i);
        result += QString("%1").arg(byte, 2, 16, QChar('0')).toUpper();

        // Add space between groups
        if ((i + 1) % bytesPerGroup == 0 && i < bytesPerLine - 1)
          result += " ";
      } else {
        result += "  ";
        if ((i + 1) % bytesPerGroup == 0 && i < bytesPerLine - 1)
