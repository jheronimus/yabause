...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    // YabaSanshiro: bundled GLFW is 3.1 which requires a non-NULL window for
    // clipboard functions (the NULL form was added in GLFW 3.4). Route through
    // the backend's own GLFW window instead of passing NULL.
    platform_io.Platform_SetClipboardTextFn = [](ImGuiContext*, const char* text) {
        ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
        if (bd && bd->Window) glfwSetClipboardString(bd->Window, text);
    };
    platform_io.Platform_GetClipboardTextFn = [](ImGuiContext*) -> const char* {
        ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
        return (bd && bd->Window) ? glfwGetClipboardString(bd->Window) : "";
    };
#ifdef __EMSCRIPTEN__
