...
//////////////////////////////////////////////////////////////////////////////
// YABA_SH2_PROF: per-PC cycle attribution profiler (interpreter core only).
//
// The Gungriffon TrueMotion decode costs ~2x more emulated time than
// mednafen even though every microbenchmark in tools/sh2timing sits at
// parity or below. That rules out a flat per-instruction cost error, so the
// question becomes "which code spends the cycles". This profiler buckets
// executed instructions and charged cycles by PC (256 bytes per bucket over
// WorkRAM-H, one catch-all bucket for anything outside it).
//////////////////////////////////////////////////////////////////////////////

#define SH2PROF_BASE    0x06000000
#define SH2PROF_SIZE    0x00100000
#define SH2PROF_SHIFT   8
#define SH2PROF_BUCKETS (SH2PROF_SIZE >> SH2PROF_SHIFT)

static int sh2prof_v = -1;
static u64 sh2prof_icount[2];
static u64 sh2prof_ccount[2];
static u32 sh2prof_hist_i[2][SH2PROF_BUCKETS + 1];
static u32 sh2prof_hist_c[2][SH2PROF_BUCKETS + 1];

int Sh2ProfEnabled(void) {
  if (sh2prof_v < 0)
    sh2prof_v = (getenv("YABA_SH2_PROF") != NULL);
  return sh2prof_v;
}

u64 Sh2ProfICount(int slave) { return sh2prof_icount[slave & 1]; }
u64 Sh2ProfCycles(int slave) { return sh2prof_ccount[slave & 1]; }

static void sh2prof_note(int slave, u32 pc, u32 cyc) {
  u32 b = SH2PROF_BUCKETS;    // catch-all bucket
  u32 off = (pc & 0x0FFFFFFF) - SH2PROF_BASE;
  if (off < SH2PROF_SIZE)
    b = off >> SH2PROF_SHIFT;
  sh2prof_icount[slave]++;
  sh2prof_ccount[slave] += cyc;
  sh2prof_hist_i[slave][b]++;
  sh2prof_hist_c[slave][b] += cyc;
}

// Writes the non-empty buckets, hottest first, and resets the histogram so
// the next dump covers only the interval that follows.
void Sh2ProfDump(const char *path, const char *tag) {
  FILE *fp;
  int cpu, i, n;
  if (!path)
    return;
  ...
  if (!fp)
    return;
  for (cpu = 0; cpu < 2; cpu++) {
    fprintf(fp, "# %s cpu=%s icount=%llu cycles=%llu\n", tag,
            cpu ? "slave" : "master",
            (unsigned long long)sh2prof_icount[cpu],
            (unsigned long long)sh2prof_ccount[cpu]);
    for (n = 0; n < 40; n++) {
      u32 best = 0, bi = 0;
      for (i = 0; i <= SH2PROF_BUCKETS; i++) {
        if (sh2prof_hist_c[cpu][i] > best) {
          best = sh2prof_hist_c[cpu][i];
          bi = i;
        }
      }
      if (!best)
        break;
      fprintf(fp, "%s %08X %u %u\n", cpu ? "S" : "M",
              (bi == SH2PROF_BUCKETS ? 0u
                                     : (u32)(SH2PROF_BASE +
                                             (bi << SH2PROF_SHIFT))),
              sh2prof_hist_i[cpu][bi], best);
      sh2prof_hist_c[cpu][bi] = 0;
      sh2prof_hist_i[cpu][bi] = 0;
    }
    memset(sh2prof_hist_c[cpu], 0, sizeof(sh2prof_hist_c[cpu]));
    memset(sh2prof_hist_i[cpu], 0, sizeof(sh2prof_hist_i[cpu]));
  }
  fclose(fp);
}
...
void SH2HandleInterrupts(SH2_struct *context) {
  if (context->NumberOfInterrupts != 0) {
    if (context->interrupts[context->NumberOfInterrupts - 1].level >
        context->regs.SR.part.I) {
      ...
      context->regs.PC = MappedMemoryReadLong(
          context->regs.VBR +
              (context->interrupts[context->NumberOfInterrupts - 1].vector
               << 2),
          NULL);

      int Vector = context->interrupts[context->NumberOfInterrupts - 1].vector;
      // LOG("[%s] Exception vecnum=(%x), PC=%08X to %08X, level=%08X",
      // (context->isslave) ? "S" : "M", Vector, oldpc, context->regs.PC,
      // level);

      LOG("[%s] %d Exception vecnum=%x, saved PC=0x%08x --- New PC=0x%08x\n",
          context->isslave ? "SH2-S" : "SH2-M", CurrentSH2->cycles,
...
static u32 FASTCALL FetchBios(u32 addr) {
  if (yabsys.extend_backup) {
    ...
    if (addr == bupaddr) {
      return 0;
    } else if (yabsys.extend_backup == 2 && addr >= BUP_TABLE_REAL_BASE &&
               addr <= BUP_TABLE_REAL_LAST) {
      // Virtual BUP table entries (real-BIOS mode): force an invalid
      // opcode so SH2undecoded dispatches to the HLE BUP functions.
      // NOTE: this must never cover real executable BIOS code; the old
      // 0x380..0x3A8 range overlapped the BIOS SYS_CHGSCUIM internals
      // and hung any game combining BUP_Init with SYS_CHGSCUIM.
      return 0;
    }
  }
...
static void FASTCALL SH2delay(SH2_struct *sh, u32 addr) {
  ...
  if ((addr & 0xC0000000) == 0xC0000000)
    sh->instruction = DataArrayReadWord(addr);
  else
#endif
    sh->instruction = MappedMemoryReadInst(
        addr, NULL); // fetchlist[(addr >> 20) & 0x0FF](addr);

  if (sh->history_enabled) {
    sh->pchistory_index++;
...
static void FASTCALL SH2undecoded(SH2_struct *sh) {
  ...
  if (yabsys.extend_backup) {
    ...
    if (sh->regs.PC == bupaddr) {
      ...
      return;
    } else if (yabsys.extend_backup == 2 && sh->regs.PC >= BUP_TABLE_REAL_BASE &&
               sh->regs.PC <= BUP_TABLE_REAL_LAST) {
      BiosHandleFunc(sh);
      return;
    }
  }
...
static void FASTCALL SH2cmpeq(SH2_struct *sh) {
  if (sh->regs.R[INSTRUCTION_B(sh->instruction)] ==
      sh->regs.R[INSTRUCTION_C(sh->instruction)])
    sh->regs.SR.part.T = 1;
  else
    sh->regs.SR.part.T = 0;
...
static void FASTCALL SH2movbl0(SH2_struct *sh) {
  ...
  sh->regs.R[n] =
      (s32)(s8)MappedMemoryReadByte(sh->regs.R[m] + sh->regs.R[0], &rcycle);
  sh->regs.PC += 2;
  sh->cycles += rcycle;
}
...
static void FASTCALL SH2movbs0(SH2_struct *sh) {
  u32 cycle = 0;
  MappedMemoryWriteByte(sh->regs.R[INSTRUCTION_B(sh->instruction)] +
                            sh->regs.R[0],
...
static void FASTCALL SH2movli(SH2_struct *sh) {
  ...
  sh->regs.R[n] = MappedMemoryReadLong(
      ((sh->regs.PC + 4) & 0xFFFFFFFC) + (disp << 2), &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movll(SH2_struct *sh) {
  u32 cycle = 0;
  sh->regs.R[INSTRUCTION_B(sh->instruction)] =
      MappedMemoryReadLong(sh->regs.R[INSTRUCTION_C(sh->instruction)], &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movll0(SH2_struct *sh) {
  u32 cycle = 0;
  sh->regs.R[INSTRUCTION_B(sh->instruction)] = MappedMemoryReadLong(
      sh->regs.R[INSTRUCTION_C(sh->instruction)] + sh->regs.R[0], &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movls(SH2_struct *sh) {
  ...
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;  // base cycle was missing (all sibling stores have it)
}
...
static void FASTCALL SH2movls0(SH2_struct *sh) {
  u32 cycle = 0;
  MappedMemoryWriteLong(sh->regs.R[INSTRUCTION_B(sh->instruction)] +
                            sh->regs.R[0],
                        sh->regs.R[INSTRUCTION_C(sh->instruction)], &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
...
static void FASTCALL SH2movwi(SH2_struct *sh) {
  ...
  sh->regs.R[n] =
      (s32)(s16)MappedMemoryReadWord(sh->regs.PC + (disp << 1) + 4, &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movwl0(SH2_struct *sh) {
  ...
  sh->regs.R[n] =
      (s32)(s16)MappedMemoryReadWord(sh->regs.R[m] + sh->regs.R[0], &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movwl4(SH2_struct *sh) {
  ...
  sh->regs.R[0] =
      (s32)(s16)MappedMemoryReadWord(sh->regs.R[m] + (disp << 1), &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movwlg(SH2_struct *sh) {
  ...
  sh->regs.R[0] =
      (s32)(s16)MappedMemoryReadWord(sh->regs.GBR + (disp << 1), &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
}
...
static void FASTCALL SH2movws0(SH2_struct *sh) {
  u32 cycle = 0;
  MappedMemoryWriteWord(sh->regs.R[INSTRUCTION_B(sh->instruction)] +
                            sh->regs.R[0],
                        sh->regs.R[INSTRUCTION_C(sh->instruction)], &cycle);
  sh->regs.PC += 2;
  sh->cycles += 1 + cycle;
...
static void FASTCALL SH2neg(SH2_struct *sh) {
  sh->regs.R[INSTRUCTION_B(sh->instruction)] =
      0 - sh->regs.R[INSTRUCTION_C(sh->instruction)];
  sh->regs.PC += 2;
  sh->cycles++;
}
...
static INLINE void SH2UBCInterrupt(SH2_struct *context, u32 flag) {
  if (15 > context->regs.SR.part.I) // Since UBC's interrupt are always level 15
  {
    ...
    context->regs.PC =
        MappedMemoryReadLong(context->regs.VBR + (12 << 2), NULL);
    LOG("interrupt successfully handled\n");
  }
  context->onchip.BRCR |= flag;
}

static u32 SH2GetRegisterValue(SH2_struct *context, int regIndex) {
  if (regIndex >= 0 && regIndex < 16) {
    return context->regs.R[regIndex];
  }
  switch (regIndex) {
    case 16: return context->regs.SR.all;
    case 17: return context->regs.GBR;
    case 18: return context->regs.VBR;
    case 19: return context->regs.MACH;
    case 20: return context->regs.MACL;
    case 21: return context->regs.PR;
    case 22: return context->regs.PC;
    default: return 0;
  }
}
...
void SH2HandleBreakpoints(SH2_struct *context) {
  ...
  for (i = 0; i < context->bp.numcodebreakpoints; i++) {
    ...
    if ((context->regs.PC == context->bp.codebreakpoint[i].addr) &&
      ...
      if (context->bp.codebreakpoint[i].hasCondition) {
        u32 regValue = SH2GetRegisterValue(context, context->bp.codebreakpoint[i].regIndex);
        if (regValue != context->bp.codebreakpoint[i].regValue) {
      ...
      if (context->bp.BreakpointCallBack)
        context->bp.BreakpointCallBack(context,
                                       context->bp.codebreakpoint[i].addr,
                                       context->bp.BreakpointUserData);
      context->bp.inbreakpoint = 0;
    }
  }
  ...
  if (context->bp.breaknow) {
    ...
    context->bp.BreakpointCallBack(context, context->regs.PC,
                                   context->bp.BreakpointUserData);
  }
}
...
FASTCALL void SH2DebugInterpreterExec(SH2_struct *context, u32 cycles) {
  ...
  // Precise-stop support for the MCP debug server: freeze this CPU
  // without executing anything while a halt is requested.
  if (context->debugHaltRequest) {
  ...
  if (context->dma_ch0.penerly != 0) {
    //      LOG("[%s] %d  add DMA Penerlty %d",CurrentSH2->isslave ? "SH2-S" :
    //      "SH2-M", CurrentSH2->cycles, context->dma_ch0.penerly);
  ...
  while (context->cycles < target_cycle) {
    ...
    // Re-check after breakpoint callback may have set halt request
    if (context->debugHaltRequest) {
      context->cycles = target_cycle;
      context->pre_cycle = 0;
      return;
    ...
    if (context->onchip.BBRA &
        (BBR_CPA_CPU | BBR_IDA_INST |
         BBR_RWA_READ)) // Break on cpu, instruction, read cycles
    {
      if (context->onchip.BARA.all ==
          (context->regs.PC & (~context->onchip.BAMRA.all))) {
        ...
        if (!(context->onchip.BRCR & BRCR_PCBA)) {
          ...
          ubcflag = BRCR_CMFCA;
               ...
               (BBR_CPA_CPU | BBR_IDA_INST |
                BBR_RWA_READ)) // Break on cpu, instruction, read cycles
    {
      if (context->onchip.BARB.all ==
          (context->regs.PC & (~context->onchip.BAMRB.all))) {
    ...
    if ((context->regs.PC & 0xC0000000) == 0xC0000000)
      context->instruction = DataArrayReadWord(context->regs.PC);
    else
#endif
      context->instruction = MappedMemoryReadInst(
          context->regs.PC, NULL); // fetchlist[(context->regs.PC >> 20) &
                                   // 0x0FF](context->regs.PC);
    ...
    if (context->history_enabled) {
      ...
      context->pchistory[context->pchistory_index & (MAX_DMPHISTORY - 1)] =
          context->regs.PC;
      context->regshistory[context->pchistory_index & (MAX_DMPHISTORY - 1)] =
          context->regs;
    }
...
FASTCALL void SH2InterpreterExec(SH2_struct *context, u32 cycles) {
  ...
  const int prof = Sh2ProfEnabled();
  const int prof_cpu = context->isslave ? 1 : 0;

  while (context->cycles < target_cycle) {
    u32 prof_pc = context->regs.PC;
    u32 prof_c0 = context->cycles;
...
#endif
      context->instruction =
          fetchlist[(context->regs.PC >> 20) & 0x0FF](context->regs.PC);

    // context->instruction = MappedMemoryReadInst(context->regs.PC, NULL);//
    // fetchlist[(context->regs.PC >> 20) & 0x0FF](context->regs.PC);
    ...
    if (prof)
      sh2prof_note(prof_cpu, prof_pc, context->cycles - prof_c0);
  }
...
u32 SH2InterpreterGetSR(SH2_struct *context) { return context->regs.SR.all; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetGBR(SH2_struct *context) { return context->regs.GBR; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetVBR(SH2_struct *context) { return context->regs.VBR; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetMACH(SH2_struct *context) { return context->regs.MACH; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetMACL(SH2_struct *context) { return context->regs.MACL; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetPR(SH2_struct *context) { return context->regs.PR; }

//////////////////////////////////////////////////////////////////////////////

u32 SH2InterpreterGetPC(SH2_struct *context) { return context->regs.PC; }

//////////////////////////////////////////////////////////////////////////////

void SH2InterpreterSetRegisters(SH2_struct *context,
...
int SH2InterpreterGetInterrupts(SH2_struct *context,
  ...
  memcpy(interrupts, context->interrupts,
...
void SH2InterpreterSetInterrupts(
    SH2_struct *context, int num_interrupts,
    const interrupt_struct interrupts[MAX_INTERRUPTS]) {
  memcpy(context->interrupts, interrupts,
         sizeof(interrupt_struct) * MAX_INTERRUPTS);
