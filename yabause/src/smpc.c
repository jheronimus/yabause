...
void SmpcRecheckRegion(void) {
   ...
   if (SmpcInternalVars->regionsetting == REGION_AUTODETECT)
   {
      ...
      // Auto-detect PAL/NTSC based on region
      if (SmpcInternalVars->regionid == REGION_ASIAPAL ||
          SmpcInternalVars->regionid == REGION_EUROPE ||
          SmpcInternalVars->regionid == REGION_CENTRALSOUTHAMERICAPAL)
      {
...
void SmpcReset(void) {
   ...
   SmpcRegs->OREG[31] = 0x0D; // initial value after reset
...
static void SmpcSSHON(void) {
   // On real hardware SSHON releases the slave SH-2 from its reset line.
   // If the slave is already running, re-issuing SSHON leaves the (already
   // released) reset line untouched, so the slave keeps executing. Emulating
   // it as an unconditional restart would reset the slave PC/registers and
   // wipe any slave program the master has already uploaded and started,
   // deadlocking games that issue a redundant SSHON (e.g. Madden NFL 97).
   // The SSHOFF -> SSHON reset idiom is preserved because SSHOFF clears
   // IsSSH2Running, allowing the following SSHON to start the slave.
...
void SmpcCKCHG352(void) {
   ...
   Vdp2Reset();
   ScuReset();
   // A clock change also stops the sound CPU (power-on default state); the
   // BIOS clock-change routine restarts it afterwards (see BiosChangeSystemClock).
   M68KStop();
   ScspReset();
...
void SmpcExec(s32 t) {
   if (SmpcInternalVars->timing > 0) {
      ...
      if (SmpcInternalVars->timing <= 0) {
         // YABA_SMPC_TRACE=1: one line per executed SMPC command. Used to
         // compare the HLE-BIOS and real-BIOS boots of the same game (the
         // slave SH-2 only runs once SSHON is seen).
         {
            static int smpc_trace = -1;
            if (smpc_trace < 0)
               smpc_trace = (getenv("YABA_SMPC_TRACE") != NULL);
            if (smpc_trace)
               printf("[SMPC f%u] COMREG=%02X ssh2=%d\n",
                      (unsigned)yabsys.frame_count, SmpcRegs->COMREG,
                      (int)yabsys.IsSSH2Running);
         }
...
void FASTCALL SmpcWriteByte(u32 addr, u8 val) {
   ...
   switch(addr) {
      ...
      case 0x1F:
         {
            static int smpc_wtrace = -1;
            if (smpc_wtrace < 0)
               smpc_wtrace = (getenv("YABA_SMPC_TRACE") != NULL);
            if (smpc_wtrace)
               printf("[SMPCW f%u] COMREG<=%02X from PC=%08X\n",
                      (unsigned)yabsys.frame_count, val,
